...
interface GetProductByIdRequest extends NextApiRequest {
  query: {
    productId: string;
    include_offers: string;
  };
}

apiRoute.get(async (req: GetProductByIdRequest, res: NextApiResponse) => {
  const productsResponse = await axios.get(
    productByIdEndpoint(req.query.productId),
    {
      params: {
        ...req.query,
      },
